...
date: 2024-08-26T09:30:00+02:00
...
`cluster-example-lb-rw`, which is configured to point to the primary (`rw`).
...
`cluster-example-lb-rw` with an external IP address, in this case,
`172.18.0.4`:

```console
NAME                    TYPE           CLUSTER-IP      EXTERNAL-IP   PORT(S)          AGE
cluster-example-lb-rw   LoadBalancer   10.96.58.150    172.18.0.4    5432:31096/TCP   3s
cluster-example-rw      ClusterIP      10.96.121.235   <none>        5432/TCP         3s
kubernetes              ClusterIP      10.96.0.1       <none>        443/TCP          20m
```

Run `kubectl describe service cluster-example-lb-rw` to get more information,
paying attention to the `Selector` part:

```console
Name:                     cluster-example-lb-rw
Namespace:                default

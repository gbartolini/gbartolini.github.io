...
via a dedicated `PGUpgrade` CRD. The cluster must be fully shut down before the upgrade can proceed;
`pg_upgrade` then runs to completion before the cluster is brought back
online. The downtime window is driven by the shutdown and restart cycle
rather than dataset size, since `pg_upgrade` uses hard links by default and
completes quickly regardless of data volume. The process is explicitly described as permanent in the
...
directly by the operator itself. Volume snapshots support both
[cold and hot backups](https://cloudnative-pg.io/docs/current/appendixes/backup_volumesnapshot#hot-and-cold-backups),
though hot backups require the WAL archive to be in place for a consistent
restore. At
...
{{< figure src="images/star-history-2026518.png"
    ...
    alt="GitHub star history chart comparing Patroni, CloudNativePG and Crunchy PGO" >}}
...
| CNCF membership | none | Sandbox, [incubation pending](https://github.com/cncf/toc/issues/1961) |
...
of only three projects to earn all five available badges (Cleaner, Chronicler,
...
_Cover picture: an elephant and a hippo plush toy from the Melbourne Zoo,
photographed by the author. A fitting pair for a comparison article — though
both are considerably better behaved than distributed systems under pressure._
